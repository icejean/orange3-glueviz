...
# data: the Orange table passed in from Spyder 
...
# def main(argv=None):
    ...
    # Added by Jean 2020/01/10, insert an initial data table widget if it's not None
...
# Added by Jean 2020/01/10, for passing in an initial table to the data table widget
init_tables = None


def insertTableWidget(canvas_window,datasets):
    if datasets is not None:
        # save reference to the table passed in for creating an instance of OWDataTable latter
        global init_tables
        init_tables = datasets
        # process the table list passing in
...
def newDataTableDesc(table):
    ...
    # Lists
    widget.keywords = []
    # One InputSignal for data table widget
    in_c = InputSignal(name='Data', type=('Orange.data.table.Table',), handler='set_dataset')
    widget.inputs = [in_c]
    # Two OutputSignal for data table widget
